...
static int adcs_sensors_status(int, char**, struct MulticallInfo *);
static int adcs_sensors_telemetry(int, char**, struct MulticallInfo *);
...
struct MulticallInfo {
   ...
   { &adcs_sensors_status, "adcs-sensors-status", "-S", 
       "Display the current status of the adcs-sensors process -S" }, 
   { &adcs_sensors_telemetry, "adcs-sensors-telemetry", "-T", 
       "Display the current KVP telemetry of the adcs-sensors process -T" }, 
   { NULL, NULL, NULL, NULL }
};

static int adcs_sensors_status(int argc, char **argv, struct MulticallInfo * self) 
{
   struct {
      uint8_t cmd;
   ...
   if ((len = socket_send_packet_and_read_response(ip, "adcs-sensors", &send, 
 ...
 */
static int adcs_sensors_telemetry(int argc, char **argv, struct MulticallInfo * self) 
{
   struct {
      uint8_t cmd;
      struct ADCSReaderStatus status;
...
static int print_usage(const char *name)
{
   ...
   printf("adcs-sensors-util multicall binary, use the following names instead:\n");
